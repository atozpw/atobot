...
const connection = async () => {
    return await mysql.createConnection({
        ...
        user: 'root',
        password: 'pohodeui',
        database: 'pdam_billing_dli'
    });
}

const getCustomer = async (id) => {
    const db = await connection();
    const [rows] = await db.query('select pel_no, pel_nama, pel_alamat from tm_pelanggan where pel_no = ?', [id]);
...
const getBills = async (id) => {
    const db = await connection();
    const [rows] = await db.query('select rek_thn, rek_bln, rek_total from tm_rekening where rek_sts = 1 and rek_byr_sts = 0 and pel_no = ?', [id]);
...
client.on('message_create', async (message) => {
    if (message.body === '!ping') {
        ...
        const greetingMessage = "Selamat datang di *AI Assistant Tirta Deli*, saya adalah Robot Kecerdasan Buatan yang hanya akan membantu Anda melakukan cek tagihan secara mandiri via Chat Only. Sekarang, cek tagihan dapat dilakukan secara mandiri via WhatsApp Chatbot dengan ketik format keyword sebagai berikut :\n- Tagihan#NomorPelanggan\n\nContoh :\n- Tagihan#12345678\n\nNote :\n- Pastikan kembali keyword Anda sudah benar.\n- Robot tidak dapat merespon yang bukan keyword.\n- Untuk layanan Telepon / Call Center : 0812-3456-7890";
        ...
        if (customer) {
            ...
            if (bills) {
                ...
                for (let bill of bills) {
                    if (i > 0) reply += `\n`;
                    reply += `Periode ${monthFormatter(bill.rek_bln)} ${bill.rek_thn}: ${rupiahFormatter(bill.rek_total)}`;
                    i++;
                }
            } else {
